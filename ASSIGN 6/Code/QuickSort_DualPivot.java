...
public class QuickSort_DualPivot<X extends Comparable<X>> extends QuickSort<X> {
	...
	public class Partitioner_DualPivot implements Partitioner<X> {
		 ...
		 * @return an array of partitions, whose length depends on the sorting method
		 *         being used.
		 */
		public List<Partition<X>> partition(Partition<X> partition) {
			...
			X v1 = xs[lo];
			X v2 = xs[hi];
			// NOTE: we are trying to avoid checking on instrumented for every time in the
			// inner loop for performance reasons (probably a silly idea).
			// NOTE: if we were using Scala, it would be easy to set up a comparer function
			// and a swapper function. With java, it's possible but much messier.
			if (helper.instrumented()) {
				helper.incrementHits(2); // XXX these account for v1 and v2.
				while (i <= gt) {
					if (helper.compare(xs, i, v1) < 0)
						helper.swap(xs, lt++, i++);
					else if (helper.compare(xs, i, v2) > 0)
						helper.swap(xs, i, gt--);
					else
						i++;
				}
	...
	public static void main(String[] args) {
		...
		while (N <= 256000) {
			...
			InstrumentedHelper<Integer> instrumentedHelper = new InstrumentedHelper<>("QuickSort_DualPivot",
					Config.setupConfig("true", "0", "0", "", ""));
